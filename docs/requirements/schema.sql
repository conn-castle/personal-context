...
CREATE TRIGGER slides_sync_bump
    AFTER INSERT OR UPDATE OR DELETE ON slides
    FOR EACH STATEMENT EXECUTE FUNCTION bump_sync_version();

CREATE TRIGGER figures_sync_bump
    AFTER INSERT OR UPDATE OR DELETE ON slide_figures
    FOR EACH STATEMENT EXECUTE FUNCTION bump_sync_version();

CREATE TRIGGER data_files_sync_bump
    AFTER INSERT OR UPDATE OR DELETE ON slide_data_files
    FOR EACH STATEMENT EXECUTE FUNCTION bump_sync_version();

CREATE TRIGGER templates_sync_bump
