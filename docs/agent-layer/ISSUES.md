...
- Issue 2026-05-07 m1g2r3a: No upgrade path for SQLite/Postgres DBs predating registry-provenance
    ...
    Description: PR #14 added `slides.source_device_id` and `source_ref` and tightened `slides.project_id`/`source_device_id` to NOT NULL with FKs to new `projects`/`devices` registry tables. The new columns and constraints are declared only inside the `CREATE TABLE IF NOT EXISTS slides` blocks (`schema/schema.sql`, `cli/internal/sqlite/sqlite_schema.sql`, `cli/internal/repository/postgres/postgres_schema.sql`). The migration runner records the canonical schema as a single version (`001_initial.sql`) via `singleFileFS` in `cli/internal/sqlite/schema.go`, and `ApplyMigrationsFromFS` only ever runs unrecorded files — so any DB that already has `001_initial.sql` recorded keeps its pre-PR layout, while repository code (`cli/internal/repository/sqlite/repository.go`, `cli/internal/repository/postgres/repository.go`) unconditionally selects and binds `source_device_id`/`source_ref`. Result on upgrade: SELECTs return `no such column`, INSERTs/UPDATEs fail with `table has no column named source_device_id`, and any read path through `pc serve`, `pc sync`, or the web UI 500s.
    Policy decision required: existing slide rows have no recorded `source_device_id`. The constraint is NOT NULL + FK to `devices(id)`, so any in-place migration must backfill every row with a value that also exists in the (newly created) `devices` table. Pick one before writing the migration:
      ...
      (b) `--default-device <id>` flag: `pc setup --upgrade --default-device my-laptop` registers `my-laptop` in `devices`, then backfills `slides.source_device_id = 'my-laptop'` for every legacy row. Preserves data; correct attribution depends on user-supplied value.
      ...
      (d) Fail-fast only: on connection open, run a probe query against `slides.source_device_id`; if missing, return an error pointing at one of (a)/(b)/(c). Lowest-cost first step that prevents silent corruption while we settle on the policy.
    ...
    Implementation work:
      ...
      - SQLite (`cli/internal/sqlite/migrations/002_registry_provenance.sql` or similar): SQLite ALTER TABLE cannot add NOT NULL without a default, cannot add FK to an existing table, and cannot drop a column without a recreate. Use the standard 12-step table-rebuild recipe inside a transaction: rename `slides`→`slides_old`, create new `slides` with full constraints, `INSERT INTO slides SELECT ..., '<backfill>' AS source_device_id, NULL AS source_ref FROM slides_old`, drop `slides_old`, recreate triggers and indexes. Make sure the chosen policy's backfill value is registered in `devices` first inside the same migration.
      - Postgres (`cli/internal/repository/postgres/migrations/002_registry_provenance.sql` or similar): can use `ALTER TABLE slides ADD COLUMN IF NOT EXISTS source_device_id TEXT`, `UPDATE slides SET source_device_id = '<backfill>' WHERE source_device_id IS NULL`, `ALTER TABLE slides ALTER COLUMN source_device_id SET NOT NULL`, then `ALTER TABLE slides ADD CONSTRAINT slides_source_device_id_fkey FOREIGN KEY ...`. Same for `source_ref` (nullable, no FK). Devices/projects rows must be inserted before the FK is added.
    ...
    Affected callers to re-test once migration lands: `pc serve` (`cli/internal/serve/server.go`), `pc sync` (`cli/internal/sync/service.go`), `pc fetch`/`pc add`/`pc edit` (provenance.go validation against registry), web UI hooks `use-slides.ts`/`use-sync-manager.ts`.
