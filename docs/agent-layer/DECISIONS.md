...
- Decision 2026-06-02 e4t6y1: web/lib/api-error.ts is the single source of truth for API error responses
    Decision: Every error response from a web API route or lib helper must be built by an `api-error.ts` helper (`badRequest`, `unauthorized`, `conflict`, `localBackendUnavailable`, etc.); no handler emits an inline `NextResponse.json({ error, code })`. The `ErrorCode` union enumerates exactly the codes the API can return, so a typo or unlisted code fails typecheck.
