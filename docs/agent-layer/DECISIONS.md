...
- Decision 2026-05-16 c4d5e6: Raw chat sources are Personal Context-owned with record-style canonical keys
    ...
    Tradeoffs: One managed raw file per chat session — multiple raw files per chat would require a separate `chat_source_files` table later. Modified re-imports overwrite the previous managed file through staged/rollback-safe promotion and still bump sync state via the `updated_at` trigger; unchanged re-imports are skipped entirely and do not bump sync state. Skipping bypasses upsert (and its `ClearDeleted: true`), so an identical re-import no longer un-deletes a soft-deleted chat — use `pc chat restore` to revive one.
