...
```
Run from: `cli/`
Notes: Enforces the hard 95% threshold and writes `coverage.out`. Excludes `internal/repository/repositorytest` (contract helper), `internal/repository/postgres`, `internal/s3client`, `internal/cloude2e` (integration-test-only, require Docker), and `internal/e2e`. Integration tests run separately with `-tags integration`.
...
```
Run from: `cli/`
Notes: Fails when any tested package drops below 95% using the profile generated by `check_coverage.sh`. Same exclusions as the aggregate coverage script. Integration-only packages (`postgres`, `s3client`, `cloude2e`) are tested with Docker via `-tags integration`.
