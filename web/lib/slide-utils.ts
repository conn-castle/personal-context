import type { SlideSummary, SlideGroup, VirtualDateSlide } from "@/lib/types";
...
/**
 * Extracts unique project IDs from an array of slides.
 *
 * @param slides - Array of slides with optional project_id.
 * @returns Sorted array of unique, non-null project ID strings.
 */
export function getUniqueProjects(
  slides: { project_id: string | null }[]
): string[] {
  const projects = new Set<string>();
  for (const slide of slides) {
    if (slide.project_id) {
      projects.add(slide.project_id);
    }
  }
...
/**
 * Groups slides by date in descending date order, ascending day_order within
 * each date. Works with any object that has `date` and `day_order` fields.
 *
 * @param slides - Array of slides to group.
 * @returns An array of { date, slides } groups.
 */
export function groupSlidesByDateDesc<
  T extends { date: string; day_order: string }
>(slides: T[]): { date: string; slides: T[] }[] {
  ...
  for (const slide of sorted) {
    const existing = groups.get(slide.date) || [];
    existing.push(slide);
    groups.set(slide.date, existing);
  }

  return Array.from(groups.entries()).map(([date, slides]) => ({
    date,
    slides,
  }));
}
...
/**
 * Groups an array of slide summaries by date.
 *
 * Assumes slides are already sorted by date DESC, day_order ASC.
 * Adjacent slides with the same date are grouped together.
 *
 * @param slides - Sorted array of slide summaries.
 * @returns An array of SlideGroup objects, one per unique date.
 */
export function groupSlidesByDate(slides: SlideSummary[]): SlideGroup[] {
  ...
  for (const slide of slides) {
    if (current === null || current.date !== slide.date) {
      current = { date: slide.date, slides: [slide] };
      groups.push(current);
    } else {
...
/**
 * Inserts virtual date-marker elements before each slide group.
 *
 * Every group receives a date marker immediately before it.
 *
 * @param groups - Slide groups (typically from groupSlidesByDate).
 * @returns Alternating VirtualDateSlide and SlideGroup elements.
 */
export function injectVirtualDateSlides(
  groups: SlideGroup[]
): (SlideGroup | VirtualDateSlide)[] {
  const result: (SlideGroup | VirtualDateSlide)[] = [];
...
/**
 * Rewrites `figures/{filename}` image sources in slide HTML to resolved URLs.
 *
