...
test("renders script-generated standalone slide HTML with bounds frame @cli-slide", async ({
  page
}) => {
  ...
  try {
    ...
    const boundary = page.locator("#slide-boundary");
    ...
    const frame = page.frameLocator("#slide-frame");
    await expect(frame.getByRole("heading", { name: "Slide A edited" })).toBeVisible();
