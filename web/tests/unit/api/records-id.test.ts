...
const mockHandlePatchRecord = vi.fn();
vi.mock("@/lib/records-handlers", () => ({
  handlePatchRecord: (...args: unknown[]) => mockHandlePatchRecord(...args),
}));
...
import { GET, PATCH, DELETE } from "@/app/api/records/[id]/route";
...
describe("GET /api/records/[id]", () => {
  ...
  it("returns record with figures and data files", async () => {
    const recordRow = {
    ...
    const figureRows = [
      {
        filename: "chart.png",
        s3_key: "records/20260301-aabbccdd/figures/chart.png",
        size: 1024,
        hash: "abc123",
        alt_text: "A chart",
        description: null,
    ...
    const dataFileRows = [
      {
        filename: "data.csv",
        s3_key: "records/20260301-aabbccdd/data/data.csv",
        size: 512,
        hash: "def456",
    ...
    // First call: record query; second: figures; third: data_files
    mockSql
    ...
    expect(body.record.id).toBe("20260301-aabbccdd");
    expect(body.record.html_content).toBe("<p>hello</p>");
  ...
  it("handles null size, hash, and description fields in figure and data file rows", async () => {
    ...
    expect(body.record.figures[0].size).toBeUndefined();
    expect(body.record.figures[0].hash).toBeUndefined();
    expect(body.record.figures[0].alt_text).toBeNull();
    expect(body.record.data_files[0].size).toBeUndefined();
    expect(body.record.data_files[0].hash).toBeUndefined();
    expect(body.record.data_files[0].alt_text).toBeUndefined();
  });
  ...
  it("returns null HTML for notes/data-only records", async () => {
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.record.html_content).toBeNull();
    expect(body.record.source_device_id).toBe("device-a");
  });

  it("returns 404 for nonexistent record ID", async () => {
    mockSql.mockResolvedValueOnce([]);

    const req = new NextRequest(
      "http://localhost/api/records/20260301-00000000"
    );
  ...
  it("returns 400 for invalid record ID format", async () => {
    const req = new NextRequest("http://localhost/api/records/bad-id");
...
describe("PATCH /api/records/[id]", () => {
  beforeEach(() => {
    mockHandlePatchRecord.mockReset();
  });

  it("delegates to handlePatchRecord with id and body", async () => {
    const { NextResponse } = await import("next/server");
    mockHandlePatchRecord.mockResolvedValue(
      NextResponse.json({ record: { id: "20260301-aabbccdd" }, sync_version: 5 })
    );
    ...
    expect(res.status).toBe(200);
    expect(mockHandlePatchRecord).toHaveBeenCalledWith(
      "20260301-aabbccdd",
      { notes: "updated" },
      "test-user-id"
    );
  ...
  it("returns 400 for invalid record ID", async () => {
    const req = new NextRequest(
      "http://localhost/api/records/bad-id",
      {
        method: "PATCH",
        body: JSON.stringify({ notes: "updated" }),
        headers: { "Content-Type": "application/json" },
...
describe("DELETE /api/records/[id]", () => {
  ...
  it("soft deletes a record and bumps S3 version", async () => {
    const deletedRow = {
      id: "20260301-aabbccdd",
  ...
  it("returns 400 for invalid record ID format", async () => {
    const req = new NextRequest("http://localhost/api/records/invalid", {
      method: "DELETE",
    });
