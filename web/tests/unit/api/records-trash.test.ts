...
import { DELETE } from "@/app/api/records/trash/route";
...
describe("DELETE /api/records/trash", () => {
  beforeEach(() => {
    mockSql.mockReset();
  ...
  it("purges trashed records and returns count", async () => {
    setupTrashMocks(
  ...
  it("returns zero when no trashed records exist", async () => {
    setupTrashMocks(
      [[{ count: 0 }], [], [], []],
  ...
  it("purges trashed records with no S3 keys to clean", async () => {
    setupTrashMocks(
      [[{ count: 1 }], [], [], []],
