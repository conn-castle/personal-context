...
describe("PATCH /api/records/[id] (handlePatchRecord)", () => {
  beforeEach(() => {
    mockSql.mockReset();
  ...
  it("updates project_id", async () => {
    ...
    mockSql.mockResolvedValueOnce([
      makeRecordRow({ project_id: "org/alpha" }),
    ]);
    // sync_version query
    ...
    const res = await handlePatchRecord(recordId, { project_id: "org/alpha" }, testUserId);
  ...
  it("returns nullable HTML and source ref from the updated row", async () => {
    mockSql.mockResolvedValueOnce([
      makeRecordRow({ html_content: null, source_ref: "vault://record" }),
    ]);
    mockSql.mockResolvedValueOnce([{ version: 5, updated_at: "2025-03-04T10:00:00.000Z" }]);
    mockSql.mockResolvedValueOnce([]);
    mockSql.mockResolvedValueOnce([]);

    const res = await handlePatchRecord(recordId, { notes: "updated" }, testUserId);
  ...
  it("updates notes and normalizes empty string to null", async () => {
    ...
    mockSql.mockResolvedValueOnce([makeRecordRow({ notes: null })]);
    // sync_version query
    ...
    const res = await handlePatchRecord(recordId, { notes: "" }, testUserId);

    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.record.notes).toBeNull();
  });
  ...
  it("updates git fields", async () => {
    ...
    mockSql.mockResolvedValueOnce([
      makeRecordRow({ git_hash: gitHash, git_remote_url: gitUrl }),
    ]);
    mockSql.mockResolvedValueOnce([{ version: 7, updated_at: "2025-03-04T10:00:00.000Z" }]);
    mockSql.mockResolvedValueOnce([]);
    mockSql.mockResolvedValueOnce([]);

    const res = await handlePatchRecord(recordId, {
      git_hash: gitHash,
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.record.git_hash).toBe(gitHash);
    expect(body.record.git_remote_url).toBe(gitUrl);
  });
  ...
  it("rejects empty body", async () => {
    const res = await handlePatchRecord(recordId, {}, testUserId);
  ...
  it("returns 400 for invalid record ID", async () => {
    const res = await handlePatchRecord("bad-id", {
      project_id: "org/alpha",
    }, testUserId);
  ...
  it("bumps S3 version after successful update", async () => {
    mockSql.mockResolvedValueOnce([makeRecordRow()]);
  ...
  it("returns 200 when S3 version bump fails after the update commits", async () => {
    ...
    mockSql.mockResolvedValueOnce([makeRecordRow({ notes: "updated" })]);
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.record.notes).toBe("updated");
