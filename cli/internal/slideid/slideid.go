...
// GenerateForDate returns a slide ID in the format {YYYYMMDD}-{8-random-hex}.
// Args: date is the local date component that must be embedded in the ID.
// Returns: a formatted slide ID or an error when random bytes cannot be read.
func GenerateForDate(date time.Time) (string, error) {
	return GenerateForDateWithReader(date, entropySource)
}

// GenerateForDateWithReader returns a slide ID using the provided entropy reader.
// Args: date is the date component; reader supplies 4 random bytes for the suffix.
// Returns: a formatted slide ID or an error for invalid input or entropy read failure.
...
// ExtractDate parses the YYYYMMDD prefix from a slide ID.
// Args: id is a slide ID in canonical format.
...
func ExtractDate(id string) (time.Time, error) {
	if !idPattern.MatchString(id) {
		return time.Time{}, fmt.Errorf("invalid slide ID: %q", id)
	}

	parts := strings.SplitN(id, "-", 2)
	parsed, err := time.ParseInLocation(dateLayout, parts[0], time.UTC)
	if err != nil {
		return time.Time{}, fmt.Errorf("parse slide ID date: %w", err)
	}

	return parsed, nil
}

// Validate reports whether an ID matches the canonical slide ID format.
// Args: id is a candidate slide ID.
