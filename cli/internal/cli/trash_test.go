...
func TestTrashWithDeletedRecords(t *testing.T) {
	setupEnv(t)
	id := addRecord(t)
...
func TestTrashListRecordsDBError(t *testing.T) {
	homeDir := setupEnv(t)
...
func TestTrashDeletedAtFormat(t *testing.T) {
	...
	for _, line := range strings.Split(out, "\n") {
		if strings.Contains(line, id) {
			// The line with the record ID should contain a timestamp like 2026-03-08T12:34:56Z
	...
	if !matched {
		t.Fatalf("expected ISO 8601 timestamp in trash output line for record %s, got %q", id, out)
	}
}
