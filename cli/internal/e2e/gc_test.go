...
func backdateDeletedAt(t *testing.T, db *sql.DB, recordID string, daysAgo int) {
	t.Helper()
...
func TestGCBoundary30Days(t *testing.T) {
	...
	ts := past.Format("2006-01-02T15:04:05.000Z")
	if _, err := db.Exec(`UPDATE records SET deleted_at = ? WHERE id = ?`, ts, recordID); err != nil {
	...
	if !strings.Contains(gcOut, "No expired trash to clean up.") {
		t.Fatalf("expected record at ~30 days to NOT be deleted, got:\n%s", gcOut)
	}
	...
	if count != 1 {
		t.Fatalf("expected record to still exist at 30-day boundary, got count=%d", count)
	}
}
...
func TestGCRemovesFigureFilesFromDisk(t *testing.T) {
	...
	dataPath := filepath.Join(homeDir, "personal-context", "data", recordID, "data.csv")
...
func TestGCLeavesActiveRecords(t *testing.T) {
	homeDir := t.TempDir()
	runPCSuccess(t, homeDir, "setup")
	...
	if !strings.Contains(gcOut, idDeleted) {
		t.Fatalf("expected gc to delete old trash record %s, got:\n%s", idDeleted, gcOut)
	}

	// Verify the active record is untouched.
	var count int
	if err := db.QueryRow("SELECT COUNT(*) FROM records WHERE id = ?", idActive).Scan(&count); err != nil {
		t.Fatalf("query active record: %v", err)
	}
	if count != 1 {
		t.Fatalf("expected active record to remain, got count=%d", count)
	}

	// Verify active record is visible via show.
	showOut := runPCSuccess(t, homeDir, "show", idActive)
	if !strings.Contains(showOut, idActive) {
		t.Fatalf("expected show to display active record, got:\n%s", showOut)
	}
}
